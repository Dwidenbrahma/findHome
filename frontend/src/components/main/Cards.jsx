...
const Card = () => {
  ...
  const tabs = ["All", "Flat", "Apartment", "Hotel", "Villa", "rent", "buy"];
  ...
  useEffect(() => {
    const fetchHomes = async () => {
      ...
      try {
        ...
        } else if (activeTab === "rent" || activeTab === "buy") {
          response = await axios.get(`${url}transaction/${activeTab}`);
        } else {
          response = await axios.get(`${url}${activeTab}`);
        }
          ...
          {tabs.map((tab) => (
            <li
              ...
              className={`px-4 py-2 rounded-md font-medium cursor-pointer transition-all duration-300 ${
                activeTab === tab
                  ? tab === "Rent"
                    ? "bg-green-600 text-white" // Rent tab style
                    : tab === "Buy"
                    ? "bg-blue-600 text-white" // Buy tab style
                    : "bg-blue-600 text-white" // Default active tab
                  : tab === "Rent"
